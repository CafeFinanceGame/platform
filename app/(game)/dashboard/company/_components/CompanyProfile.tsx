...
export const CompanyProfile: React.FC<Props> = (props) => {
  ...
  const metadata = {
    [CompanyType.COFFEE_COMPANY]: {
      ...
      avatar_alt:
        "https://i.pinimg.com/736x/53/2f/0c/532f0c643d553e0449275c0b80d481aa.jpg",
    },
    [CompanyType.MACHINE_COMPANY]: {
      icon: <PiWashingMachine />,
      label: "Machine Company",
      avatar_alt:
        "https://i.pinimg.com/736x/26/30/8e/26308ef78d21841abdb37b7bf55feead.jpg",
    },
    [CompanyType.MATERIAL_COMPANY]: {
      icon: <GiSugarCane />,
      label: "Material Company",
      avatar_alt:
        "https://i.pinimg.com/736x/b4/7d/1f/b47d1f6ade41dc7d2ab0aad77afd2105.jpg",
    },
  } as any;
  ...
  const RoleField = ({ role }: { role: CompanyType }) => {
    ...
    return (
      <div className="flex flex-row gap-2 items-center">
        {metadata[role].icon}
        <p className="capitalize text-xs text-default-500">
          {metadata[role].label}
        </p>
      </div>
    );
  ...
  if (isLoading) return <Skeleton className="w-full rounded-3xl h-64" />;
    ...
    return (
      <div className="flex flex-[1] flex-col gap-2 rounded-3xl border-2 border-default-200 p-4">
        <Image
          ...
          src={company.avatar || metadata[company.role].avatar_alt}
        />
    ...
    return (
      <div className="flex flex-[2] flex-col gap-2 items-end justify-center">
        {fields.map(({ label, value }) => (
          <div
            key={nanoid()}
            className="w-full max-w-screen-sm flex flex-row justify-between items-center"
          >
